...
NSVGrasterizer* nsvgCreateRasterizer(void)
{
	NSVGrasterizer* r = (NSVGrasterizer*)kmalloc(sizeof(NSVGrasterizer));
...
void nsvgDeleteRasterizer(NSVGrasterizer* r)
{
	...
	if (r->points) kfree(r->points);
	if (r->points2) kfree(r->points2);
	if (r->scanline) kfree(r->scanline);
...
static NSVGmemPage* nsvg__nextPage(NSVGrasterizer* r, NSVGmemPage* cur)
{
	...
	newp = (NSVGmemPage*)kmalloc(sizeof(NSVGmemPage));
...
static void nsvg__addPathPoint(NSVGrasterizer* r, float x, float y, int flags)
{
	...
	if (r->npoints+1 > r->cpoints) {
		...
		r->points = (NSVGpoint*)krealloc(r->points, sizeof(NSVGpoint) * r->cpoints);
...
static void nsvg__duplicatePoints(NSVGrasterizer* r)
{
	if (r->npoints > r->cpoints2) {
		...
		r->points2 = (NSVGpoint*)krealloc(r->points2, sizeof(NSVGpoint) * r->cpoints2);
...
static void nsvg__addEdge(NSVGrasterizer* r, float x0, float y0, float x1, float y1)
{
	...
	if (r->nedges+1 > r->cedges) {
		...
		r->edges = (NSVGedge*)krealloc(r->edges, sizeof(NSVGedge) * r->cedges);
				   ...
				   unsigned char* dst, int w, int h, int stride)
{
	...
	if (w > r->cscanline) {
		r->cscanline = w;
		r->scanline = (unsigned char*)krealloc(r->scanline, w);
