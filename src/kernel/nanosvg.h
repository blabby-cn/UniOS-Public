...
{
	NSVGparser* p;
	p = (NSVGparser*)kmalloc(sizeof(NSVGparser));
	if (p == NULL) goto error;
	memset(p, 0, sizeof(NSVGparser));

	p->image = (NSVGimage*)kmalloc(sizeof(NSVGimage));
...
static void nsvg__deleteStyles(NSVGstyleDeclaration* style) {
	while (style) {
		...
		kfree(style->className);
		kfree(style->propertiesText);
		kfree(style);
		style = next;
	}
}
...
static void nsvg__addPoint(NSVGparser* p, float x, float y)
{
	if (p->npts+1 > p->cpts) {
		...
		p->pts = (float*)krealloc(p->pts, p->cpts*2*sizeof(float));
		if (!p->pts) return;
	}
	p->pts[p->npts*2+0] = x;
	p->pts[p->npts*2+1] = y;
...
static NSVGgradient* nsvg__createGradient(NSVGparser* p, const char* id, const float* localBounds, float *xform, signed char* paintType)
{
	...
	grad = (NSVGgradient*)kmalloc(sizeof(NSVGgradient) + sizeof(NSVGgradientStop)*(nstops-1));
...
static void nsvg__addShape(NSVGparser* p)
{
	...
	shape = (NSVGshape*)kmalloc(sizeof(NSVGshape));
...
static void nsvg__addPath(NSVGparser* p, char closed)
{
	...
	path = (NSVGpath*)kmalloc(sizeof(NSVGpath));
	if (path == NULL) goto error;
	memset(path, 0, sizeof(NSVGpath));

	path->pts = (float*)kmalloc(p->npts*2*sizeof(float));
...
error:
	if (path != NULL) {
		if (path->pts != NULL) kfree(path->pts);
		kfree(path);
	}
}
...
static unsigned int nsvg__parseHexPairN(const char *s, int len)
{
	unsigned int v = 0;
	int i;
	for (i = 0; i < len; i++) {
...
static unsigned int nsvg__parseHexPair(const char *s)
{
	return nsvg__parseHexPairN(s, 2);
}
...
static unsigned int nsvg__parseColorHex(const char* str)
{
	...
	if (str[0] == '#' && len == 7) {
		r = nsvg__parseHexPair(&str[1]);
		g = nsvg__parseHexPair(&str[3]);
		b = nsvg__parseHexPair(&str[5]);
		return NSVG_RGB(r, g, b);
	}
	if (str[0] == '#' && len == 4) {
		r = nsvg__parseHexPairN(&str[1], 1) * 17;
		g = nsvg__parseHexPairN(&str[2], 1) * 17;
		b = nsvg__parseHexPairN(&str[3], 1) * 17;
		return NSVG_RGB(r, g, b);
	}
...
static unsigned int nsvg__parseColorRGB(const char* str)
{
	 ...
	 
	{
		...
		int parsed = 0;
		for (i = 0; i < 3; i++) {
			while (*p && nsvg__isspace(*p)) p++;
			rgbi[i] = (unsigned int)strtol(p, (char**)&p, 10);
			if (p && (i<2 ? *p==',' : *p==')')) { parsed++; if (i<2) p++; }
			else break;
		}
		if (parsed == 3) {
			for (i = 0; i < 3; i++) { if (rgbi[i] > 255) rgbi[i] = 255; }
...
{
	int i;
	NSVGgradientData* grad = (NSVGgradientData*)kmalloc(sizeof(NSVGgradientData));
	if (grad == NULL) return;
...
static void nsvg__parseGradientStop(NSVGparser* p, const char** attr)
{
	...
	grad->nstops++;
	grad->stops = (NSVGgradientStop*)krealloc(grad->stops, sizeof(NSVGgradientStop)*grad->nstops);
...
static void nsvg__content(void* ud, const char* s)
{
	...
	while (*s) {
		...
		while (*s && *s != '{') {
			...
			style = (NSVGstyleDeclaration*)kmalloc(sizeof(NSVGstyleDeclaration));
			if (style == NULL)
				continue;
...
NSVGpath* nsvgDuplicatePath(NSVGpath* p)
{
    ...
    res = (NSVGpath*)kmalloc(sizeof(NSVGpath));
    if (res == NULL) goto error;
    memset(res, 0, sizeof(NSVGpath));

    res->pts = (float*)kmalloc(p->npts*2*sizeof(float));
